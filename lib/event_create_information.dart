...
class _EventCreationInformationState extends State<EventCreationInformation>  {
  ...
  final TextEditingController event_description_setter = TextEditingController();
  ...
  final TextEditingController event_dateandtime_setter = TextEditingController(); // od
  final TextEditingController event_dateandtime_end_setter = TextEditingController(); // do 🆕
  final TextEditingController event_price_setter = TextEditingController();
  final TextEditingController event_participants_setters = TextEditingController();
  final TextEditingController event_visibility_setter = TextEditingController();

  List<String> allCategories = ["Šport", "Hudba", "Party", "Škola", "Work"];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
     ...
     Column(crossAxisAlignment: CrossAxisAlignment.start,children: [
       ...
       Text("popis"),
       TextField(controller: event_description_setter,
       decoration: const InputDecoration(
         labelText: "Name of event",
         border: OutlineInputBorder(),
       ),
       ...
       Text("miesto"),
       TextField(controller: event_place_setter,
       ...
       ElevatedButton(
         onPressed: () {
           ...
           widget.event.price = double.tryParse(event_price_setter.text) ?? 0.0;
           widget.event.participants = int.tryParse(event_participants_setters.text) ?? 0;
           widget.event.dateFrom = DateTime.tryParse(event_dateandtime_setter.text);
           widget.event.dateTo = DateTime.tryParse(event_dateandtime_end_setter.text);
